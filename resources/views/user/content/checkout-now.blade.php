...
@endsection
@section('content')
<form action="{{ route('checkoutNow') }}" method="POST" enctype="multipart/form-data">
  @csrf
    ...
    <div class="col-lg-6 mb-4 border p-4 rounded">
      ...
      <div class="alamat">
         <div class="mb-3 d-flex justify-content-between">
            ...
            <h6>{{ $item->nama }}</h6>
            <p>Rp {{ $item->harga }}</p>
            <input name="barang" type="hidden" value="{{ $item->id }}">
            @endforeach
        </div>
        <div class="mb-3">
          <label for="">Total Item</label>
          <input type="text" class="form-control w-25" name="qty" value="{{ $qty }}" readonly>
          <input type="hidden" name="sub" value="{{ encrypt($total) }}">
        </div>
        <hr class="mb-3">
